...
func changeSlice(s []int) []int {
	...
	fmt.Printf("【func】s val %v     ptr %p  len %v  cap %v \n", s, &s, len(s), cap(s))
	s = append(s, 3) //未触发扩容 slice = [10, 1, 3]
	fmt.Printf("【apd3】s val %v   ptr %p  len %v  cap %v \n", s, &s, len(s), cap(s))
	s = append(s, 4) //触发扩容  所以移动到了新地址  slice = [10, 1, 3]  new = [10, 1, 3, 4]
	fmt.Printf("【apd4】s val %v ptr %p  len %v  cap %v \n", s, &s, len(s), cap(s))
